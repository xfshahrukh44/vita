...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark">Channels</h1>
</div>
</div>
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-tools">
                    <button class="btn btn-success" id="add_program" data-toggle="modal" data-target="#addChannelModal">
                    ...
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                        ...
                        <tbody>
                            ...
                            <tr role="row" class="odd">
                                <td class="{{'name'.$channel->id}}">{{$channel->name}}</td>
                                <td>
                                    @can('isSuperAdmin')
                                        <!-- Edit -->
                                        <a href="#" class="editButton" data-id="{{$channel->id}}">
                                            <i class="fas fa-edit blue ml-1"></i>
                                        </a>
                                        <!-- Delete -->
                                        <a href="#" class="deleteButton" data-id="{{$channel->id}}">
                            ...
                            @else
                            <tr>
                                <td colspan="4"><h6 align="center">No channel(s) found</h6></td>
                            </tr>
                            @endif
                        </tbody>
                        <tfoot>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        ...
        <form method="POST" action="{{route('channel.store')}}">
            @include('admin.channel.channel_master')
...
<!-- Edit view -->
<div class="modal fade" id="editChannelModal" tabindex="-1" role="dialog" aria-labelledby="editChannelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
        ...
        <form id="editForm" method="POST" action="{{route('channel.update', 1)}}">
            <!-- hidden input -->
            @method('PUT')
...
<div class="modal fade" id="deleteChannelModal" tabindex="-1" role="dialog" aria-labelledby="deleteChannelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        ...
        <form id="deleteForm" method="POST" action="{{route('channel.destroy', 1)}}">
            <!-- hidden input -->
            @method('DELETE')
            @csrf
...
<script>
    $(document).ready(function(){
        ...
        $('.editButton').on('click', function(){
            var id = $(this).data('id');
            $('#editForm').attr('action', "{{route('channel.update', 1)}}");
            $('#hidden').val(id);
        ...
        $('.deleteButton').on('click', function(){
            var id = $(this).data('id');
            $('#deleteForm').attr('action', "{{route('channel.update', 1)}}");
            $('#deleteForm .hidden').val(id);
            
            $('#deleteChannelModalLabel').text('Delete Channel: ' + $('.name' + id).html() + "?");
